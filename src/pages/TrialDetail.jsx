...
export default function TrialDetail({ nctId, disease, go }) {
  ...
  // Initialize Leaflet map once trial is loaded
  useEffect(() => {
    if (!trial || !mapRef.current || typeof window.L === "undefined") return;
    const L = window.L;
    if (!mapObj.current) {
    ...
    const validSites = (userLoc ? sortByDistance(trial.locations, userLoc.lat, userLoc.lng) : trial.locations)
    ...
    if (userLoc) {
      L.circle([userLoc.lat, userLoc.lng], { color: "#0d7a7a", fillColor: "#0d7a7a", fillOpacity: 0.15, radius: 50000 }).addTo(map);
    }

    if (validSites.length > 0) {
      const group = L.featureGroup(validSites.map(s => L.marker([s.lat, s.lng])));
      try { map.fitBounds(group.getBounds().pad(0.3)); } catch { /* map may not be ready */ }
    }

    setSites(userLoc ? sortByDistance(trial.locations, userLoc.lat, userLoc.lng) : trial.locations);
  }, [trial, userLoc]);

  async function handleZipSearch() {
    const loc = await geocodeZip(zip);
    if (!loc) { alert("Could not find that zip code."); return; }
    setUserLoc(loc);
  }
